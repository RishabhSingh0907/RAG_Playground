...
from src.generation.chat_manager import ChatManager
from src.generation.prompt_builder import PromptBuilder
from src.generation.citation_extractor import CitationExtractor
from src.generation.llm_provider import create_llm_provider, LLMConfig
# from src.monitoring import evaluate_response
from src.monitoring import RagasEvaluator
...
# Apply custom CSS for app layout and sidebar styling
st.markdown(
    """
    <style>
    :root {
        --rag-header-offset: 0px;
        --rag-tab-offset: 0px;
        --rag-panel-height: calc(100vh - 170px);
    }

    /* Keep the primary navigation available while tab content scrolls. */
    div[data-testid="stTabs"] > div[role="tablist"] {
        position: sticky;
        top: var(--rag-tab-offset);
        z-index: 20;
        background: #0E1117;
    ...
    /* Make every tab body its own scroll frame below the sticky tab bar. */
    div[data-testid="stTabs"] > div[role="tabpanel"] {
        max-height: var(--rag-panel-height);
        overflow-y: auto;
    ...
    div[data-testid="stTabs"] > div[role="tabpanel"]::-webkit-scrollbar-thumb,
    [data-testid="stSidebar"] > div:first-child::-webkit-scrollbar-thumb {
        background: rgba(255, 255, 255, 0.28);
        border-radius: 999px;
    }

    /* Treat the configuration viewer as an independent scrollable panel. */
    [data-testid="stSidebar"] > div:first-child {
        max-height: 100vh;
        overflow-y: auto;
    }

    .rag-chat-panel-title {
...
if "chat_last_chunks" not in st.session_state:
    st.session_state.chat_last_chunks = []
...
if "ragas_evaluator" not in st.session_state:
    st.session_state.ragas_evaluator = RagasEvaluator(model="gpt-oss:20b")
...
def render_chat_playground():
    """Render chat playground with retrieval + generation."""
    ...
    stats = manager.get_stats()
    if stats["total_chunks"] == 0:
        st.info("No chunks available. Please ingest documents first.")
        return
    ...
    with right:
        ...
        available_models = st.session_state.chat_manager.get_available_models()

        if not available_models:
            st.warning("⚠️ No Ollama models detected. Make sure Ollama is running.")
            available_models = ["mistral"]

        default_model = st.session_state.get("chat_model", available_models[0])
        ...
        max_tokens = st.slider(
            ...
            min_value=128,
            max_value=4096,
            value=int(st.session_state.get("chat_max_tokens", 1024)),
            step=128,
            key="chat_max_tokens"
        )
        ...
        system_prompt = st.text_area(
            ...
            value=st.session_state.get(
                "chat_system_prompt",
                "Answer ONLY using the retrieved context. Cite evidence when possible. If insufficient information exists, clearly say so."
            ),
            height=180,
        ...
        if st.button("🧹 Clear Conversation", key="clear_chat"):
            st.session_state.chat_manager.clear_session()
            st.rerun()

    with left:
        st.subheader("Conversation")

        current_session = getattr(st.session_state.chat_manager, "current_session", None)
        ...
        user_prompt = st.chat_input("Ask questions about your documents...")

        if user_prompt:
            st.session_state.chat_manager.add_user_message(user_prompt)
            ...
            with st.chat_message("assistant"):
                with st.spinner("Retrieving and generating response..."):
                    try:
                        ...
                        pipeline = RetrievalPipelineV2(manager, settings=settings)
                        results = pipeline.search(user_prompt)
                        ...
                        if hasattr(provider, "health_check"):
                            ...
                            if isinstance(ok, dict):
                                ok = ok.get("healthy", True)
                            if not ok:
                                st.error("LLM provider unavailable.")
                                return
                        ...
                        with st.expander(
                            ...
                            with c3:
                                ...
                                st.caption(indicator(precision))
...
def main():
    ...
    with col1:
        st.title("🚀 RAG Playground - Retrieval & Generation Studio")
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(
        ["📤 Upload & Ingest", "🔍 Browse Chunks", "🔎 Retrieval", "💬 Chat Playground", "📊 Statistics", "⚙️ Configuration", "📝 Logs"]
    )
